...
Proyek ini bertujuan untuk mengembangkan sistem rekomendasi makanan berbasis machine learning bagi pasien penderita diabetes. Sistem ini menggabungkan dua pendekatan utama, yaitu content-based filtering yang memanfaatkan informasi nutrisi makanan, dan collaborative-like filtering yang menyesuaikan rekomendasi berdasarkan profil fisiologis pasien. Dengan memanfaatkan dua dataset terpisah—data nutrisi makanan dan data pasien—sistem ini diharapkan dapat memberikan rekomendasi makanan yang tidak hanya sehat secara umum, namun juga sesuai dengan kondisi medis individu.

## 1. Project Overview

Diabetes melitus, khususnya tipe 2, merupakan salah satu penyakit kronis yang paling banyak diderita di seluruh dunia dan menjadi penyebab utama berbagai komplikasi kesehatan seperti penyakit kardiovaskular, gagal ginjal, serta gangguan penglihatan. Salah satu aspek penting dalam pengelolaan diabetes adalah pengaturan pola makan, karena konsumsi makanan yang tidak sesuai dapat menyebabkan lonjakan kadar gula darah yang berbahaya. Oleh karena itu, penentuan asupan makanan yang tepat dan individualistik menjadi kebutuhan mendesak dalam praktik klinis dan gaya hidup penderita diabetes[1].

Namun, dalam praktiknya, sebagian besar penderita diabetes tidak memiliki akses langsung terhadap layanan ahli gizi, atau kurang memiliki pengetahuan untuk mengidentifikasi makanan yang aman dikonsumsi sesuai kondisi fisiologis mereka[2]. Sementara itu, sistem rekomendasi makanan yang tersedia saat ini masih bersifat umum dan tidak mempertimbangkan profil kesehatan individual, seperti kadar gula darah, tekanan darah, indeks massa tubuh (BMI), atau kadar HbA1c. Hal ini menyebabkan rekomendasi makanan yang diberikan sering kali kurang relevan atau bahkan berisiko memperburuk kondisi pasien[3].

Dengan kemajuan teknologi machine learning dan data sains, kini dimungkinkan untuk membangun sistem rekomendasi makanan yang lebih cerdas dan personal. Dengan menggabungkan pendekatan content-based filtering (berbasis nutrisi makanan) dan collaborative-like filtering (berbasis kemiripan antar-profil pasien), sistem ini dapat memberikan saran makanan yang tidak hanya sehat, tetapi juga sesuai dengan parameter fisiologis pasien diabetes secara spesifik[4].

### *Mengapa Masalah Ini Harus Diselesaikan?*

1. Tingginya Risiko Komplikasi Diabetes: Pengelolaan pola makan yang buruk merupakan salah satu penyebab utama memburuknya kondisi diabetes. Tanpa rekomendasi yang tepat, pasien berisiko mengalami lonjakan gula darah yang dapat memicu komplikasi serius.

2. Kesenjangan Pengetahuan dan Akses: Tidak semua pasien memiliki akses terhadap ahli gizi atau mampu menginterpretasi informasi gizi secara benar. Sistem rekomendasi yang bersifat otomatis dapat membantu menjembatani kesenjangan tersebut.

3. Kurangnya Personalization dalam Sistem Saat Ini: Sistem rekomendasi makanan yang tersedia secara umum masih belum mempertimbangkan kondisi medis dan fisiologis pengguna, sehingga kurang akurat dan berpotensi tidak efektif bagi penderita diabetes.

### *Bagaimana Masalah Ini Harus Diselesaikan?*

Masalah ini dapat diselesaikan dengan pendekatan sistematis melalui pengembangan sistem rekomendasi makanan berbasis data medis dan gizi. Adapun langkah-langkah strategis yang diterapkan dalam proyek ini adalah:

1. Pengumpulan dan Pemahaman Data: Mengintegrasikan dua sumber data penting, yaitu data nutrisi makanan dan data fisiologis pasien diabetes.

2. Pra-pemrosesan dan Normalisasi: Membersihkan data dari nilai hilang, menyamakan format, serta menormalkan nilai numerik untuk memastikan kualitas input model.
...
4. Evaluasi dan Visualisasi: Menyediakan analisis visual untuk mendukung pemahaman pengguna serta mengevaluasi relevansi dan personalisasi hasil rekomendasi.

5. Pengembangan Sistem Lanjutan: Integrasi sistem ke dalam antarmuka pengguna seperti web atau aplikasi mobile, dan penyesuaian berkelanjutan berdasarkan feedback pengguna atau evaluasi ahli medis.
...
### Latar Belakang  
- Pola hidup tidak sehat, konsumsi fast food tinggi, menjadi penyebab Diabetes Mellitus.  
...
- **Jumlah Nilai DuplikatD**:![alt text](image-2.png)
...
Fungsi foods_recommendations() menerima beberapa parameter penting: nama sebagai referensi makanan, similarity_data sebagai matriks kemiripan antar makanan, dan items, yaitu DataFrame yang memuat setidaknya informasi nama, kategori, dan gula. Parameter k menentukan jumlah maksimum makanan yang akan direkomendasikan. Sistem ini sangat berguna dalam memberikan saran makanan yang serupa dengan preferensi pengguna, namun tetap sehat karena mengedepankan kadar gula yang rendah sebagai prioritas utama.
...
Fungsi rekomendasi_makanan() bekerja dengan menerima input berupa nilai probabilitas (antara 0 dan 1) yang merepresentasikan risiko diabetes pasien. Berdasarkan nilai ini, fungsi akan memfilter makanan dalam dua kategori utama: makanan yang aman (dengan skor_risiko lebih rendah dari probabilitas) dan makanan berisiko (dengan skor_risiko lebih tinggi). Sistem kemudian menampilkan beberapa sampel makanan dari masing-masing kategori secara acak untuk memberikan wawasan yang seimbang antara apa yang direkomendasikan dan apa yang sebaiknya dihindari oleh pasien.
...
## Kesimpulan Dampak terhadap Business Understanding
...
## Daftar Pustaka
[1]Munir, M. M., Pujianto, A., Aulia, H., & Lamuru, M. (2023). Optimisasi Algoritma Genetika dengan Particle Swarm Optimization (PSO) untuk Sistem Rekomendasi Diet Gizi bagi Penderita Diabetes. Jurnal RESTIA, 1(2), 38–48. https://doi.org/10.30787/restia.v1i2.1289

[2]Nasution, S. R., Andreswari, D., & Wahyu, T. (2019). IMPLEMENTASI NAÏVE BAYES CLASSIFIER DAN SIMPLE ADDITIVE WEIGHTING (SAW) UNTUK PEMILIHAN MENU DIET PENYAKIT DIABETES MELLITUS. Jurnal Rekursif, 7(1). http://ejournal.unib.ac.id/index.php/rekursif/1

[3]Prasanti, L. D., & Utomo, D. T. (2024). PERANCANGAN SISITEM PENDUKUNG KEPUTUSAN REKOMENDASI MENU MAKANAN PADA PENDERITA DIABETES MELLITUS MENGGUNAKAN METODE SIMPLE ADDITIVE WEIGHTING. Jurnal Kecerdasan Buatan Dan Teknologi Informasi, 3(1), 11–16. https://doi.org/10.69916/jkbti.v3i1.62
